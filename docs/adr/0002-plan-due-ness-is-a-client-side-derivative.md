...
- `frontend/src/lib/serviceplan.ts` remains the home of the rule; its public surface narrows to the eight doors, the exported `planCmp`, and the `ServicePlan`/`Limits` shapes, `limit_keys`, the `plans` store, and CRUD (spec #323 is the execution record; the `unlink_plan` unlink landed as its backend half).
