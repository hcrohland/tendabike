...
pub(super) fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/{shop}/details", get(get_shop_details))
...
async fn list_shops(user: RequestSession, State(pool): State<DbPool>) -> ApiResult<Vec<Shop>> {
    let mut store = pool.begin().await?;
    Ok(Shop::get_all_for_user(&user.get_id(), &mut store)
        .await
        .map(Json)?)
}
...
async fn create_shop(
    ...
    let mut store = pool.begin().await?;
    let shop = ShopId::create(name, description, &user, &mut store).await?;
    store.commit().await?;
...
async fn get_shop_details(
    Path(shop_id): Path<i32>,
    user: RequestSession,
    State(pool): State<DbPool>,
) -> ApiResult<tb_domain::ShopSummary> {
    let mut store = pool.begin().await?;
    let shop_id = ShopId::get_for_read(shop_id, &user, &mut store).await?;
    let summary = shop_id.get_details(&user, &mut store).await?;
    Ok(Json(summary))
}
...
async fn create_subscription(
    ...
    let mut store = pool.begin().await?;
    let subscription = SubscriptionId::create(shop_id.into(), message, &user, &mut store).await?;
    store.commit().await?;
...
async fn list_my_subscriptions(
    ...
    let mut store = pool.begin().await?;
    let subscriptions = ShopSubscription::get_for_user(&user, &mut store).await?;
