...
export function mapObject<V>(field: keyof V, del?: (v: V) => boolean): (a: map<V>, b: V) => map<V> {
    return (map, obj) => {
        if (del && del(obj))
            delete map[getid(obj, field)]
        else
            map[getid(obj, field)] = obj;
        return map;
    }
}
...
export function mapable<V>(field: keyof V, prepfn?: (v: any) => V, delfn?: (v: V) => boolean) {
    ...
    return {
        subscribe,
        setMap: (arr: V[]) => { set(arr
                .map(prepfn1)
                .reduce(mapObject(field, delfn), {})) },
        updateMap: (arr: V[]) => update(map => arr.map(prepfn1).reduce(mapObject(field, delfn), map)),
    };
}
