<script lang="ts">
  ...
  $: gears = filterValues($parts, (p) => p.what == $category.id && ! p.disposed_at).sort(by("last_used"))
  $: bin = filterValues($parts, (p) => p.what == $category.id && p.disposed_at != undefined).sort(by("last_used"))
...
{#if $category }
  ...
  <Row class="p-sm-2">
    ...
    {:else}
      ...
      {:else}
         You have no {$category.name} assigned to any activity on Strava. Please do so to get started.
      {/if}
    {/each}
  </Row>
