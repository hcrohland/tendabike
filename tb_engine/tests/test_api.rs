    ...
    U: Into<std::borrow::Cow<'u, str>>,
{
    ...
    serde_json::from_str::<T>(&response.body_string().expect("body is no string"))
...
fn post_and_delete_activity() {
    ...
    act.gear = None;
    let (mut act_new, ass): (Activity, Assembly) =
        reqjson(&client, Method::Post, "/activ/", &act, Status::Created);
    assert!(act_new.id != 0.into());
    assert_eq!(act_new.start, act.start);
    assert!(ass.is_empty());

    act_new.gear = Some(1.into());
    let ass: Assembly = reqjson(
        &client,
        Method::Put,
        format!("/activ/{}", act_new.id),
        &act_new,
        Status::Ok,
    ); //Should use response header?
    let part1 = ass.part(1.into()).unwrap();
